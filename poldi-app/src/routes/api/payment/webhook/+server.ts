  ...
  try {
    ...
    switch (event.type) {
      case 'checkout.session.completed': {
        ...
        // Fallback: try to match by customer email (for Payment Links created in Stripe Dashboard)
        if (!userId && session.customer_email) {
          const db = getDatabase('./data');
          const user = await db.users.findByEmail(session.customer_email);
          if (user) {
            userId = user.id;
            console.log(`Matched payment to user ${userId} via email: ${session.customer_email}`);
          }
        }
        ...
        if (userId) {
          ...
          console.warn('Payment completed but could not identify user:', {
